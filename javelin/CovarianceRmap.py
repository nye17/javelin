...
__all__ = ['CovarianceRmap']


from numpy import *
from numpy.linalg import cholesky, LinAlgError
from javelin.gp.GPutils import regularize_array, trisolve, square_and_sum
from javelin.gp.linalg_utils import diag_call, dpotrf_wrap
from javelin.gp.incomplete_chol import ichol, ichol_continue
from spear import spear

class CovarianceRmap(object):

    """
    C=CovarianceRmap(relative_precision, **params)
    ...
    :Arguments:
        ...
        -   `**params` : sigma, tau, lags, wids, scales

    """


    def __init__(self, relative_precision = 1.0E-15, rank_limit=0, **params):
    ...
    def cholesky(self, x, idx, apply_pivot = True, observed=True, nugget=None, regularize=True, rank_limit=0):
        """

        U = C.cholesky(x, idx[, observed=True, nugget=None, rank_limit=00])


        {'pivots': piv, 'U': U} = \
        C.cholesky(x, idx, apply_pivot = False[, observed=True, nugget=None])


        Computes incomplete Cholesky factorization of self(x,x,idx,idx), without
        ...
        :Arguments:
            ...
            -   `idx`: The input index array on which to evaluate the covariance.
        ...
        if regularize:
            ...
            idx=regularize_array(idx)

        # Number of points in x.
        N_new = x.shape[0]

        # diagonal
        diag = self.__call__(x, idx, y=None, idy=None, 
                regularize=False, observed=observed)
    ...
    def observe(self, obs_mesh, obs_idmesh, obs_V, output_type='r'):
        """
        Observes self on obs_mesh and obs_idmesh with observation 
        variance obs_V.
        ...
        if not self.observed:
            ...
            if output_type != 's':
                obs_dict = self.cholesky(obs_mesh, obs_idmesh, apply_pivot = False, nugget = obs_V, regularize=False, rank_limit = self.rank_limit)
            else:
    ...
    def __call__(self, x, idx, y=None, idy=None, observed=True, regularize=True, 
            return_Uo_Cxo=False):

        if (y is x) and (idy is idx) :
            symm=True
        else:
            symm=False
        ...
        if y is None:
            ...
            for i in xrange(lenx):
                ...
                this_idx = idx[i].reshape((1,-1))
                V[i] = self.eval_fun(this_x, this_x, this_idx, this_idy, 
                        **self.params)
            if self.observed and observed:
                sqpart = empty(lenx,dtype=float)
                Cxo = self.eval_fun(self.obs_mesh, x, self.obs_idmesh, idx,
                        **self.params)
            ...
            # ====================================================
            if symm:
                C=self.eval_fun(x,x,idx,idx,symm=True,**self.params)
            ...
            # ======================================
                ...
                C = self.eval_fun(x,y,idx,idy,**self.params)
                ...
                if self.observed and observed:
                    ...
                    Cyo = self.eval_fun(self.obs_mesh, self.obs_idmesh, y, idy, 
                            **self.params)
