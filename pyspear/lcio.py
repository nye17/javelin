# Last-modified: 25 Oct 2011 01:49:09 PM

""" 
Functions related to the processing of quasar light curves.
...
def combinelc(lctxt,lcdata):
    """ Write plain old column files into the ZY light curve data file.
...
def writelc(lightcurvearray, lcdata):
    ...
    for ilc in range(nlc):
        ...
        npt = len(jd)
        f.write(str(npt)+"\n")
        f.write("\n".join([" ".join([str(jd[i]), str(pt[i]), str(er[i])]) for i in range(npt)]))
        f.write("\n")
    f.close()
